...
def test_one_sided_vinokur_functions():
    ds = hippogryph.single_sided_factor(1.0e-6, 1, 16, output=dev_null)
    ...
    ds = hippogryph.single_sided_factor(2.0, 1, 16, output=dev_null)
    assert ds is None

    ds = hippogryph.single_sided_factor(1.0e-6, 1, 16, output=dev_null, max_iterations=2)
    assert ds is None
